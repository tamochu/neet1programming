...
public class Exam4Easy {
	...
	public static void main(String[] args) {
		...
		for (int x = 0; x < chars.length; x++) {
			...
			for (int i = 0; i < angou.length; i++) {
				indexes[i] = charToIndex(/* i番目の文字 */);
			}

			// intの列すべてにxを足す（x文字ずらす）
			...
			// 文字数を超えたら0から
			for (int i = 0; i < indexes.length; i++) {
				if (indexes[i] >= /* 文字「a～-」の数 */) {
					indexes[i] -= /* 文字「a～-」の数 */;
				}
			}

			// intの配列を文字の配列に直す
			char[] hukugou = new char[/* 暗号の長さ */];
			for (int i = 0; i < indexes.length; i++) {
	...
	public static int charToIndex(char c) {
		for (int index = 0; index < /* 文字「a～-」の数 */; index++) {
			if (c == chars[index]) {
				return /* 何番目の文字かを示すint */;
			}
		}
		// どれでもなかった場合とりあえず0を返す
		return 0;
